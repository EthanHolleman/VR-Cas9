...
rule build_database:
    ...
    input:
        ...
        target='output/{run}/reslice/{seq_name}.reslice.fa'
    output:
...
rule discover_targets:
    conda:
        '../envs/java.yml'
    input:
        database='output/{run}/flashFryDatabases/{seq_name}',
        target='output/{run}/reslice/{seq_name}.reslice.fa',
        ff='software/FlashFry-assembly-1.12.jar'
    output:
        'output/{run}/flashFryTargets/{seq_name}.targets'
    shell:'''
...
rule score_targets:
    ...
    input:
        ff='software/FlashFry-assembly-1.12.jar',
        discovered_targets='output/{run}/flashFryTargets/{seq_name}.targets',
        database='output/{run}/flashFryDatabases/{seq_name}'
    output:
